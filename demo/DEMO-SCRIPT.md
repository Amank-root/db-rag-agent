...
| 1:20 | **THE PAUSE** — approval card on screen | "Now the part most agents skip: rollback is irreversible, so it STOPS and asks — here's the evidence, the exact action, and how to verify. Nothing happens until I approve." |
...
| 2:35 | Type: `Which endpoint cost us the most errors last week?` | "Second loop: analytics. It writes SQL, runs it in the sandbox, renders the chart — and if I asked it to write back, it would pause for approval again." |
| 2:55 | Outro | "MCP tools, sandboxed code, subagents, approvals, persistent sessions — TrueForge did the work. Thanks." |

**Rescue plans**
- Approval doesn't appear → invoke `rollback_deploy` explicitly from the chat ("propose the rollback call") — the gate fires on the write tool.
- Sandbox hiccup → local fallback still runs the script; mention "isolated execution" verbally.
